...
loads_wide_df = loads_wide_df.xs(20, level='day_of_month',axis=1)
...
def test_kshape():
    ...
    print(loads.shape)
    fig, ax = plt.subplots(6, 2, figsize=(18, 15))
    for yi in range(num_cluster):
        plt.subplot(6, 2, yi + 1)
        for xx in loads[loads[:, 0] == yi]:
            plt.plot(xx.ravel()[0:95], "k-", alpha=.3)
        cluster_center = scaler.inverse_transform(ks.cluster_centers_[yi, :].reshape(1, -1))
        print(scaler.mean_t.shape)
        plt.plot(cluster_center.ravel()[0:95], "r-")
    ...
    for i in range(num_cluster):
        cluster_center = scaler.inverse_transform(ks.cluster_centers_[i, :].reshape(1, -1))
        plt.plot(cluster_center.ravel()[0:95], label="cluster %s" % (str(i + 1)))
    plt.ylabel('用电量/kW')
    plt.legend()
    plt.xlabel('采样点')
    plt.show()
